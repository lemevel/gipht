...
%gcc quadphase.c -O -lm -o quadphase 
cmd1='../src/quadphase psp_11176_21540_ort.pha  1420 1230 qsp_11176_21540_ort.pha qsp_11176_21540_ort.phalst'
% usage: quadphase     input.pha                nx   ny   output.pha              output.phalst              ithresh[8] minpix[16]
...
%unix(sprintf('%s psp_11176_21540_ort.phalst 8 16 ../demoQ/coh_11176_21540_ort.oct 1',cmd1)) % use only pixels with coh > 1st percentile
...
ilist = read_i2('qsp_11176_21540_ort.phalst',4);
% modify quadphase.c so that first line of binary file to include NX and NY
% Also, modify quadphase.c to ignore coherence file

% write a program in C to expand .phalist into .pha file, as done in the
% following lines
whos ilist
i1=ilist(:,1);  % Index to row of first pixel in patch
j1=ilist(:,2);  % Index to col of first pixel in patch
kw=ilist(:,3);  % width (and height) of square patch
qp=ilist(:,4);  % ohase value coded -128 to +127 such that 256 DN = 1 cycle
