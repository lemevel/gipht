...
cmd1='../src/pha2qls3.a64 psp_11176_21540_ort.pha  1420 1230 qsp_11176_21540_ort.pha grx_11176_21540_ort.i2 gry_11176_21540_ort.i2 qsp_11176_21540_ort.qls'
...
npatch = numel(qp)
figure;
nf=nf+1;h(nf)=figure;hist(double(qp(:)),64);axis tight;
...
%     i0 = double(i1(k));
%     j0 = double(j1(k));
%     for i=double(i1(k):i2(k));  % changed to double PES
%         for j=double(j1(k):j2(k)          );  %changed to double
...
for k=1:npatch;
    ivec=double(i1(k):i2(k));jvec=double(j1(k):j2(k));
    r(jvec,ivec)=qp(k); % add in DC offset
    r(jvec,ivec)=bsxfun(@plus,r(jvec,ivec),gx(k)*(ivec-ivec(1)));
     r(jvec,ivec)=bsxfun(@plus,r(jvec,ivec),gy(k)*(jvec-jvec(1))');
end
...
%% % display patches in 3d
patchy=[j1 j1 j2+1 j2+1]'; patchx=[i1 i2+1 i2+1 i1]';
id=double(i2+1-i1);jd=double(j2+1-j1);
patchz=([qp qp+gx.*id qp+gx.*id+gy.*jd qp+gy.*jd]');

figure;patch(patchy,patchx,patchz,patchz);
shading interp;view(14.5,68)
