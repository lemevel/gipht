function [pest, psig, mse, dmod, tfit, pfit, sfitl, sfitu] = temporal_adjustment3(data,data_sigma,tm,ts,tbreaks,tfunc,metaparams)
%function [pest, psig, mse, dmod, tfit, pfit, sfitl, sfitu] = temporal_adjustment3(data,data_sigma,tm,ts,tbreaks,tfunc,metaparams)
...
mparams = numel(time_function6(tfunc, min([tm; tu]), tbreaks, metaparams));
...
switch tfunc
    ...
    otherwise
        ...
        for i=1:ndat
            tfm = time_function6(tfunc, tm(i), tbreaks, metaparams);
            tfs = time_function6(tfunc, ts(i), tbreaks, metaparams);
            for j=1:mparams
                G(i,j) = tfs(j)-tfm(j);
            end
        end % loop over data
...
if ndat < 2
    ...
    switch tfunc
        ...
        otherwise
            ...
            dsu = 0;
            for i=2:numel(tfit)
                dt = time_function6(tfunc, tfit(i), tbreaks, metaparams) - time_function6(tfunc, tfit(i-1), tbreaks, metaparams);
                for j=1:mparams
