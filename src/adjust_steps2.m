function [pest, psig, mse, dmod, intmod, tfit, pfit, sfit,rfit] = adjust_steps(data,data_sigma,tm,ts,imast,islav,tbreaks,DD)
...
if ndat < 2
    ...
    for i=1:numel(tfit)
        dm = 0;
        for j = 1:mparam
            dm = dm + heaviside1(tfit(i)-tbreaks(j)) * pest(j);
        end
        pfit(i) = dm;
