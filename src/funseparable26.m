function varargout = funseparable26(varargin)
%function varargout = funseparable26(varargin)
%
...
elseif nin == 2 && nout == 2 && isstruct(varargin{1}) == 1 && isstruct(varargin{2}) == 1
    ...
    % number of epochs
    me = numel(get_parameter_index('epoch',PST.names))/11;
    if round(me) ~= me
    ...
    for i = 1:ndata
        ...
        DDS(i,DST.kmast(i)) = -1; % subtract master
    ...
    if numel(char(PST.datafilename)) > 0
        ...
        if fexist(char(PST.datafilename)) == 1
            ...
            kabaqus = get_parameter_index('AQ_Excess_Pressure_in_Pa________',PST.names);
            if abs(PST.p1(kabaqus)) > 0.
                PSTAQ.mname    = PST.datafilename;
                PSTAQ.pressure = PST.p1(kabaqus+0); % Pressure increase in Pascals
                PSTAQ.xcen     = PST.p1(kabaqus+1); % Easting coordinate of Center in meters
                PSTAQ.ycen     = PST.p1(kabaqus+2); % Northing coordinate of Center in meters
                PSTAQ.depth    = PST.p1(kabaqus+3); % Depth of center in meters
                %PSTAQ.EYM = E;
    ...
    % call looks like this:
    %   rng            = feval(DST,PST,TST); % return set of scalar ranges
    %   [rng,DST]      = feval(DST,PST,TST); % also return DST structure
...
elseif nin == 3 ...
    ...
    ksundisk = 46;
    if (abs(pg(ksundisk+3)) > 0)
        switch idatatype1
            case 0 % observable is phase
                usundisk1 = penny(DST.x',DST.y'...
                    ,pg(ksundisk+0),pg(ksundisk+1),nu,E ...
                    ,pg(ksundisk+2),pg(ksundisk+4),pg(ksundisk+3));
            case -1 % observable is gradient
...
%     pg(kcomsol+2)
    if (abs(pg(kcomsol+2)) > 0)
        ...
        PSTCS.xcen = pg(kcomsol+0); % Easting Coordinate
        PSTCS.ycen = pg(kcomsol+1); % Northing Coordinate
%         PSTCS.POR  = pg(kcomsol+2); % porosity
%         PSTCS.PER  = pg(kcomsol+3); % permeability
        PSTCS.POR  = 10^pg(kcomsol+2); % log10 porosity
        PSTCS.PER  = 10^pg(kcomsol+3); % log10 permeability
        ...
        cmod = -1.0* (ucs.*DST.uvx + vcs.*DST.uvy + wcs.*DST.uvz);
    ...
    if abs(pg(kabaqus)) > 0.
        PSTAQ.mname    = PST.datafilename;
        PSTAQ.pressure = pg(kabaqus+0); % Pressure increase in Pascals
%         PSTAQ.xcen     = pg(kabaqus+1); % Easting coordinate of Center in meters
%         PSTAQ.ycen     = pg(kabaqus+2); % Northing coordinate of Center in meters
%         PSTAQ.depth    = pg(kabaqus+3); % Depth of center in meters
        PSTAQ.verbose  = 0;
...
%         end
    ...
    % sum Okada and Mogi displacements
    %u = umogi1 + umogi2 + uokada1 + uYangPS + upinel;
    ...
    % combine time and space dependence
    %rng1 = tdif .* gmod + nui + bas;              % unwrapped phase model in m
    rng1 = tdif .* gmod + nui + bas + cmod;  % unwrapped phase model in m
